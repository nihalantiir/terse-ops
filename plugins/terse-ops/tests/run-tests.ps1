...
Invoke-Case BLOCK "PowerShell Remove-Item -Recurse -Force" 'Remove-Item -Recurse -Force build/'
Invoke-Case BLOCK "PowerShell ri alias, short flags"    'ri -r -f build/'
Invoke-Case ALLOW "PowerShell Remove-Item without -Recurse" 'Remove-Item -Force file.txt'
Invoke-Case ALLOW "PowerShell Copy-Item -Recurse -Force is not a delete" 'Copy-Item -Recurse -Force src/ dst/'
Invoke-Case ALLOW "danger-ok overrides PowerShell delete" 'TERSE_OPS_DANGER_OK=1 Remove-Item -Recurse -Force build/'
