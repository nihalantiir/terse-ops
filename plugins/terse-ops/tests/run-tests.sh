...
run_case BLOCK "PowerShell Remove-Item -Recurse -Force" 'Remove-Item -Recurse -Force build/'
run_case BLOCK "PowerShell ri alias, short flags"      'ri -r -f build/'
run_case ALLOW "PowerShell Remove-Item without -Recurse" 'Remove-Item -Force file.txt'
run_case ALLOW "PowerShell Copy-Item -Recurse -Force is not a delete" 'Copy-Item -Recurse -Force src/ dst/'
run_case ALLOW "danger-ok overrides PowerShell delete" 'TERSE_OPS_DANGER_OK=1 Remove-Item -Recurse -Force build/'
